...
class Boleto
{
        ...
        if (file_exists($caminho)) {
            ...
            if (is_array($handle)) {
                ...
                foreach ($handle as $linha) {
                    if ($inicio) {
                        if (substr($linha, 0, 9) != '02RETORNO') {
                            throw new \Exception("O arquivo está em um formato incorreto");
                        }
                        $inicio = false;
                    }

                    $seg = substr($linha, 0, 1);

                    if (($seg == '1') and (strlen($linha) >= 400)) {

                        $linha = " $linha";
                        $data[] = $this->processarDetalhe($linha);
    ...
    protected function processarHeaderArquivo($linha)
    {
        ...
        */
        ...
        $vetor["data_gravacao"] = substr($linha, 95, 6); //9 Data da Gravação: Informe no formado “DDMMAA”
        $vetor["densidade_gravacao"] = substr($linha, 101, 8); //01600000
        $vetor["num_aviso_bancario"] = substr($linha, 109, 5);
        $vetor["data_credito"] = substr($linha, 380, 6); // “DDMMAA”
