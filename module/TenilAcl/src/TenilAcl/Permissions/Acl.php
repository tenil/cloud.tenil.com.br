...
namespace TenilAcl\Permissions;
...
class Acl extends ClassAcl {
    ...
    protected function loadRoles() {
        foreach ($this->roles as $role) {
            if ($role->getParent()) {
                $this->addRole(new Role($role, new Role($role->getParent())));
            } else {
    ...
    protected function loadPrivileges() {
        foreach ($this->privileges as $privilege) {
            $roleName = $privilege->getRole()->getNome();
            $resourceName = $privilege->getResource()->getNome();
            $privilegeName = $privilege->getNome();
